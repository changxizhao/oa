...
<%@ include file="/systeminfo/init_wev8.jsp"%>
	...
	<%
		...
		String yjzz_sql = "select distinct sskf,kfmc from uf_ydzczjb zj,uf_kfgl kf where zj.sskf = kf.id " + sql_where ;
		log.writeLog("export yjzz_sql = " +yjzz_sql);
		yjzz_rs.execute(yjzz_sql);
		while(yjzz_rs.next()){
			AssestExportEntity entity = new AssestExportEntity();
			int id = Util.getIntValue(yjzz_rs.getString("sskf"));
			String kfmc = Util.null2String(yjzz_rs.getString("kfmc"));
			entity.setId(id);
			entity.setName(kfmc);
			log.writeLog("export value = " + id + "," + kfmc);
			list.add(entity);
		}
		...
		for(AssestExportEntity entity : list){
			// sql server
     		//String sql = "select convert(char(7),czny) as czny,sum(yzjje) as zje from uf_ydzczjb where sskf = "+entity.getId() + sql_where + " group by convert(char(7),czny)";
			// oracle
			String sql = "select substr(czny, 0, 7) as czny,sum(yzjje) as zje from uf_ydzczjb where sskf = "+entity.getId() + sql_where + " group by substr(czny, 0, 7)";
		...
		list.add(total);
		log.writeLog("库房 export list = " + JSONObject.toJSONString(list));
		String[] title = new String[]{"ID","所属库房","一月","二月","三月","四月","五月","六月","七月","八月","九月","十月","十一月","十二月"};
